...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function Pointer() {
      ...
      this.down = false;
      this.moved = false;
      this.color = { r: 0.9, g: 0.92, b: 0.98 };
    }
    ...
    function getWebGLContext(targetCanvas) {
      ...
      const formatRGBA = isWebGL2
        ...
        : getSupportedFormat(context, context.RGBA, context.RGBA, halfFloatTexType);

      const formatRG = isWebGL2
        ? getSupportedFormat(context, context.RG16F, context.RG, halfFloatTexType)
        : getSupportedFormat(context, context.RGBA, context.RGBA, halfFloatTexType);

      const formatR = isWebGL2
        ? getSupportedFormat(context, context.R16F, context.RED, halfFloatTexType)
        : getSupportedFormat(context, context.RGBA, context.RGBA, halfFloatTexType);
    ...
    const displayShaderSource = `
      ...
      void main () {
        vec3 c = texture2D(uTexture, vUv).rgb;
          ...
          float diffuse = clamp(dot(n, l) + 0.72, 0.75, 1.0);
          c *= diffuse;
        #endif

        float alpha = clamp(max(c.r, max(c.g, c.b)) * 1.18, 0.0, 0.9);
    ...
    const vorticityShader = compileShader(
      ...
      void main () {
        ...
        vec2 velocitySample = texture2D(uVelocity, vUv).xy;
        velocitySample += force * dt;
        velocitySample = min(max(velocitySample, -1000.0), 1000.0);
        gl_FragColor = vec4(velocitySample, 0.0, 1.0);
      }
      `,
    );
    ...
    const pressureShader = compileShader(
      ...
      void main () {
        ...
        float divergenceSample = texture2D(uDivergence, vUv).x;
        float pressureSample = (L + R + B + T - divergenceSample) * 0.25;
        gl_FragColor = vec4(pressureSample, 0.0, 0.0, 1.0);
      }
      `,
    );
    ...
    function generateColor() {
      const tones = [
        { r: 0.92, g: 0.95, b: 1.0 },
        { r: 0.84, g: 0.88, b: 0.94 },
        { r: 0.78, g: 0.81, b: 0.87 },
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[1] overflow-hidden">
      <canvas
        ref={canvasRef}
        className="block h-full w-full opacity-60 mix-blend-screen"
        aria-hidden="true"
      />
    </div>
  );
